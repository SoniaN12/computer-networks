...
def add_contact(username, contact):
    ...
    #Adds a contact for the specified user in the database.
    ...
    try:
        ...
        cursor.execute("SELECT username FROM users WHERE username=?", (contact,))
        if cursor.fetchone() is None:
            print(f"[ERROR] Contact '{contact}' does not exist.")
            return False

        # Insert the contact into the contacts table
        cursor.execute('INSERT INTO contacts (user_id, user, contact) VALUES (?,?, ?)', (username, contact))
        conn.commit()
        print(f"[INFO] Contact '{contact}' added for user '{username}'.")
        return True
    ...
    if mode == "register":
        try:
            cursor.execute("INSERT INTO users(username, password)VALUES(?,?)",
                           (username, hash_password(password)))  # executing a query
